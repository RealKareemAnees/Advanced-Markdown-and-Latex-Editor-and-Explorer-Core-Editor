...
/**
 * Strategy for duplicating multiple nodes.
...
export class DuplicateMultipleNodesHandlerStrategy implements ActionHandlerStrategyInterface {
    ...
    handle(
        ...
        if (!selectedNodes || selectedNodes.length === 0) {
            throw new Error("No nodes selected for duplication");
        }
        return memory.duplicateMultipleNodes(selectedNodes).ArrayRepresentation;
    }
}
